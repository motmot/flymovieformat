...
class TestFMF(unittest.TestCase):
    ...
    def test_mmap(self):
        for filename in fmf_filenames:
            if filename.endswith("test_rgb8.fmf") or filename.endswith(
                "test_rgb32f.fmf"
            ):
                continue
            fmf = FlyMovieFormat.FlyMovie(filename)
...
class TestExporterPlugins(unittest.TestCase):
    ...
    def test_plugins(self):
        ...
        for filename in fmf_filenames:
            ...
            width_height = (fmf.get_width(), fmf.get_height())
...
def get_test_suite():
    ts = unittest.TestSuite(
        [unittest.makeSuite(TestFMF), unittest.makeSuite(TestExporterPlugins),]
    )
    return ts
