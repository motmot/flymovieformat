    ...
    def get_saver(self, wx_parent, format, width_height):
        ...
        def OnImageSequenceBrowse(event):
            if wx_parent is not None:
                ...
                dlg3 = wx.FileDialog(
                    wx_parent, "Select basename...", os.getcwd(), "", "", wx.SAVE
                )
                try:
        ...
        if wx_parent is not None:
            ...
            wx.EVT_BUTTON(
                dlg2,
                xrc.XRCCTRL(dlg2, "image_basename_browse").GetId(),
                OnImageSequenceBrowse,
            )
            wx.EVT_TEXT(dlg2, xrc.XRCCTRL(dlg2, "image_basename").GetId(), OnTextEvent)
            wx.EVT_TEXT(
                dlg2, xrc.XRCCTRL(dlg2, "image_number_format").GetId(), OnTextEvent
            )
            wx.EVT_CHOICE(
                dlg2, xrc.XRCCTRL(dlg2, "image_extension_choice").GetId(), OnChoiceEvent
            )
            wx.EVT_BUTTON(
                dlg2, xrc.XRCCTRL(dlg2, "cancel_button").GetId(), OnCancelImageSequence
            )
            wx.EVT_BUTTON(
                dlg2, xrc.XRCCTRL(dlg2, "save_button").GetId(), OnSaveImageSequence
            )
...
class GenericSaverPlugin(object):
    def get_saver(self, wx_parent, format, widthheight):
        ...
        if wx_parent is not None:
            ...
            wildcard += "|All files (*.*)|*.*"
            dlg2 = wx.FileDialog(
                wx_parent, "Save movie as...", os.getcwd(), "", wildcard, wx.SAVE
            )
            filename = None
            try:
    ...
    def sub_get_saver(self, wx_parent, filename, format, widthheight):
        class FmfSaver(object):
            def __init__(self, filename, format):
                self.fmf_file = FlyMovieFormat.FlyMovieSaver(
                    filename, version=3, format=format, bits_per_pixel=bpp(format)
                )
