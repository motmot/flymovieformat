    ...
    def __init__(self, file, check_integrity=False):
        ...
        if version == 1:
            ...
            if sys.version_info.major <= 2:
                ...
                self.format = r(format_len).decode()
            self.bits_per_pixel = unpack(
                BITS_PER_PIXEL_FMT, r(size(BITS_PER_PIXEL_FMT))
            )[0]
    ...
    def _read_next_frame(self, allow_partial_frames=False):
        ...
        if len(data) < self.bytes_per_chunk:
            if allow_partial_frames:
                ...
                warnings.warn(
                    "appended %d bytes (to a total of %d), image will be corrupt"
                    % (missing_bytes, self.bytes_per_chunk)
                )
            else:
                raise NoMoreFramesException(
                    "short frame "
                    "(%d bytes instead of %d)" % (len(data), self.bytes_per_chunk)
                )
        timestamp_buf = data[: self.timestamp_len]
        (timestamp,) = struct.unpack(TIMESTAMP_FMT, timestamp_buf)

        if (
            self.format in ("MONO8", "RAW8")
            or self.format.startswith("MONO8:")
            or self.format.startswith("RAW8:")
        ):
            frame = numpy.frombuffer(data[self.timestamp_len :], numpy.uint8)
            frame.shape = self.framesize
        elif self.format in ("YUV422"):
            frame = numpy.frombuffer(data[self.timestamp_len :], numpy.uint16)
            frame.shape = self.framesize
        elif self.format in ("RGB8"):
            frame = numpy.frombuffer(data[self.timestamp_len :], dtype=numpy.uint8)
            h, w = self.framesize
            frame.shape = (h, w * 3)
        elif self.format in ("MONO32f", "RAW32f") or self.format.startswith("MONO32f:"):
            frame = numpy.frombuffer(data[self.timestamp_len :], numpy.float32)
            frame.shape = self.framesize
            ...
            raise NotImplementedError(
                "Reading not implemented for %s format" % (self.format,)
            )
...
def mmap_flymovie(*args, **kwargs):
    ...
    my_dtype = numpy.dtype([("timestamp", "<f8"), ("frame", dtype_str, fmf.framesize)])
    assert my_dtype.itemsize == fmf.bytes_per_chunk

    offset = fmf.chunk_start
    shape = (fmf.get_n_frames(),)
    ra = numpy.memmap(
        fmf.filename, dtype=my_dtype, offset=offset, mode="r", shape=shape
    )
    fmf.close()
    return ra
...
class FlyMovieSaver:
    ...
    def __init__(
        self, file, version=1, seek_ok=True, format=None, bits_per_pixel=None,
    ):
        ...
        if version == 3:
            ...
            if sys.version_info.major <= 2:
                is_string = isinstance(format, basestring)  # Python 2.x
            else:
                is_string = isinstance(format, str)  # Python 3.x
    ...
    def add_frame(
        self, origframe, timestamp=nan, error_if_not_fast=False, fast_write=True
    ):
        ...
        if self.framesize is None:
            ...
            if self.framesize != thisshape:
                raise ValueError(
                    "frame shape is now %s, but it used to be %s"
                    % (str(thisshape), str(self.framesize))
                )
        ...
        if fast_write and hasattr(origframe, "dump_to_file"):
            ...
            if fast_write and error_if_not_fast:
                raise AttributeError(
                    "fast write failed, no dump_to_file or tofile method"
                )
            b2 = frame.tobytes()
            if len(b2) != self._bytes_per_image:
                raise ValueError(
                    "expected buffer of length %d, got length %d (shape %s)"
                    % (self._bytes_per_image, len(b2), str(thisshape))
                )
            self.file.write(b2)
        self.n_frames += 1
    ...
    def _do_v1_header(self, frame):
        ...
        buf = struct.pack(FRAMESIZE_FMT, self.framesize[0], self.framesize[1])
        self.file.write(buf)

        bits_per_image = self.framesize[0] * self.framesize[1] * self.bits_per_pixel
        if bits_per_image % 8 != 0:
            raise ValueError(
                "combination of frame size and bits_per_pixel make non-byte aligned image"
            )
